/**
 * Base Write GB command — write data to a GB (national standard) tag.
 *
 * Mirrors Python `base_writeGb.py`.
 */
...
/** Return-code descriptions for the write-gb response. */
...
/** Options for {@link MsgBaseWriteGb}. */
export interface WriteGbOptions {
  /** Filter parameter. */
  filter?: Parameter;
  /** Hex access password. */
  hexPassword?: string;
...
/**
 * Write data to a GB (national standard) tag.
 */
export class MsgBaseWriteGb extends Message {
  ...
  constructor(
    ...
    start: number,
    hexWriteData: string,
    options?: WriteGbOptions,
  ) {
    super();
    this.mt_8_11 = EnumG.Msg_Type_Bit_Base;
    this.msgId = EnumG.BaseMid_WriteGb;
  ...
  override pack(): void {
    ...
    if (this.initMode !== undefined) {
      buffer.putUint8(0x04);
      buffer.putUint8(this.initMode);
    }
