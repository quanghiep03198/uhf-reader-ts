/**
 * Base Lock GB command — lock a memory area of a GB (national standard) tag.
 *
 * Mirrors Python `base_lockGb.py`.
 */
...
/** Return-code descriptions for the lock-gb response. */
...
/** Options for {@link MsgBaseLockGb}. */
export interface LockGbOptions {
  /** Filter parameter. */
  filter?: Parameter;
  /** Hex access password. */
  hexPassword?: string;
...
/**
 * Lock a memory area of a GB (national standard) tag.
 */
export class MsgBaseLockGb extends Message {
  ...
  constructor(
    ...
    area: number,
    lockParam: number,
    options?: LockGbOptions,
  ) {
    super();
    this.mt_8_11 = EnumG.Msg_Type_Bit_Base;
    this.msgId = EnumG.BaseMid_LockGb;
