/**
 * Base Inventory GB command — start a GB (national standard) tag inventory.
 *
 * Mirrors Python `base_inventoryGb.py`.
 */
...
/** Return-code descriptions for the inventory-gb response. */
...
/** Options for {@link MsgBaseInventoryGb}. */
export interface InventoryGbOptions {
...
/**
 * Start a GB (national standard) tag inventory on selected antennas.
 */
export class MsgBaseInventoryGb extends Message {
   ...
   */
  constructor(antennaEnable: number, inventoryMode: number, options?: InventoryGbOptions) {
    super();
    this.mt_8_11 = EnumG.Msg_Type_Bit_Base;
    this.msgId = EnumG.BaseMid_InventoryGb;
