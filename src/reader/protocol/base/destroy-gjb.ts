/**
 * Base Destroy GJB command — permanently destroy a GJB (military standard) tag.
 *
 * Mirrors Python `base_destroyGJb.py`.
 */
...
/** Return-code descriptions for the destroy-gjb response. */
...
/** Options for {@link MsgBaseDestroyGJb}. */
export interface DestroyGJbOptions {
  /** Filter parameter. */
  filter?: Parameter;
  /** Hex access password. */
  hexPassword?: string;
...
/**
 * Permanently destroy (kill) a GJB (military standard) tag.
 */
export class MsgBaseDestroyGJb extends Message {
   ...
   */
  constructor(antennaEnable: number, hexPassword: string, options?: DestroyGJbOptions) {
    super();
    this.mt_8_11 = EnumG.Msg_Type_Bit_Base;
    this.msgId = EnumG.BaseMid_DestroyGJb;
