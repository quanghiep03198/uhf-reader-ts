...
/** Options for setting Wi-Fi IP configuration. */
export interface WifiIpOptions {
  ip?: string;
  mask?: string;
  gateway?: string;
  dns1?: string;
...
/** Command to set Wi-Fi IP configuration. */
export class MsgAppSetWifiIp extends Message {
  /** Auto IP mode (0 = DHCP, 1 = static). */
  autoIp: number;
  /** Static IP. */
  ip: string | undefined;
  /** Subnet mask. */
  ...
  /** Hotspot ID. */
  hotId: number | undefined;

  constructor(autoIp: number, opts?: WifiIpOptions) {
    super();
    this.mt_8_11 = EnumG.Msg_Type_Bit_App;
    this.msgId = EnumG.AppMid_SetWifiIp;
    this.autoIp = autoIp;
    this.ip = opts?.ip;
    this.mask = opts?.mask;
  ...
  override pack(): void {
    ...
    buffer.putUint32BE(this.hotId ?? 0);
