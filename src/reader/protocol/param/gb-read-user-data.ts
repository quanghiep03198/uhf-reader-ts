...
/**
 * Parameter for reading user data from GB/T tags.
 */
export class ParamGbReadUserData extends Parameter {
  /** Child memory area code. */
  childArea: number;
  ...
  /**
   * @param childArea Child memory area code.
   * @param start     Word offset to start reading from.
   * @param dataLen   Number of bytes to read.
   */
  constructor(childArea: number, start: number, dataLen: number) {
    super();
  ...
  toBytes(): number[] {
    ...
    buffer.putUint8(this.childArea);
  ...
  fromBytes(data: number[]): void {
    ...
    this.childArea = buffer.readUint8();
  ...
  toString(): string {
    return `(${this.childArea}, ${this.start}, ${this.dataLen})`;
  }
}
